...
for (const [file, img] of Object.entries(mapping)) {
  ...
  if (fs.existsSync(fp)) {
    ...
    // Some might use imgPlaceholder from "@/assets/..."
    content = content.replace(/import imgPlaceholder from "@\/assets\/[^"]+";/g, `const imgPlaceholder = "${img}";`);
    
    fs.writeFileSync(fp, content);
  }
}
console.log("Updated case study files properly");
